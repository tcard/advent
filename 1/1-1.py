...
# second digit and the third digit (2) matches the fourth digit. 1111 produces 4
# because each digit (all 1) matches the next. 1234 produces 0 because no digit
# matches the next. 91212129 produces 9 because the only digit that matches the
# next one is the last digit, 9. What is the solution to your captcha?
...
def reverse_captcha(digits):
    ...
    def next(i):
        return digits[i + 1 if i < len(digits) - 1 else 0]
